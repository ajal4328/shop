...
class Usercreat extends StatefulWidget {
  const Usercreat({super.key});

  @override
  State<Usercreat> createState() => _UsercreatState();
}
...
class _UsercreatState extends State<Usercreat> {
  ...
  final FirebaseFirestore _regstrr=FirebaseFirestore.instance;

  Regiter()async{
    if(!_register.currentState!.validate())return;

    try{
      UserCredential userCred=await FirebaseAuth.instance.createUserWithEmailAndPassword(
          email: email.text.trim(), password: password.text.trim());
      await _regstrr.collection('user').doc(userCred.user!.uid).set({
